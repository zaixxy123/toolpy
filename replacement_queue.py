...
class ReplacementQueueManager(QObject):
    ...
    def start_capture(self):
        if self.is_recording:
            return
        self.clear()
        self.capture_directory = Path(tempfile.mkdtemp(prefix="ToolPy_Replacement_Queue_"))
    ...
    def finish_capture(self, parent):
        if not self.is_recording:
            return False
        if self.count == 0:
            QMessageBox.warning(parent, "No Images Captured", "Copy at least one image first.")
            return False
        self._disconnect_clipboard()
        self.is_recording = False
        self.queue_locked = True
        self.current_index = 0
        self.state_changed.emit("Ready to replace")
    ...
    def replace_selected(self, parent):
        if not self.queue_locked:
            QMessageBox.warning(parent, "Queue Not Ready", "Start Capture, copy images, then click Finish Capture.")
            return False
        if not self.has_remaining:
            QMessageBox.warning(parent, "Queue Complete", "Every captured image has already been used.")
            return False
        ...
        try:
            ...
            if selection.InlineShapes.Count > 0:
                self._replace_inline_shape(selection.InlineShapes(1), replacement_file)
            else:
                try:
                ...
                if shape_range is None or shape_range.Count == 0:
                    QMessageBox.warning(parent, "No Image Selected", "Select one image inside Microsoft Word first.")
                    return False
                self._replace_floating_shape(shape_range(1), replacement_file)
            ...
            if self.current_index >= self.count:
                ...
                self.state_changed.emit("Ready — select the next Word image")
                self.current_changed.emit(self.current_index + 1, self.count)
            return True

        except Exception as error:
            QMessageBox.critical(parent, "Replace Error", f"ToolPy could not replace the selected image.\n\nError:\n{error}")
            return False
    ...
    def _capture_current_clipboard(self):
        ...
        if mime_data.hasUrls():
            for url in mime_data.urls():
                ...
                if image.isNull():
                    continue
                images.append(image)
                signature_parts.append(f"file:{file_path.resolve()}:{file_path.stat().st_mtime_ns}")

        if not images and mime_data.hasImage():
            image = self.clipboard.image()
            if not image.isNull():
                images.append(image)
                signature_parts.append("image:" + self._image_hash(image))
        ...
        now = time.monotonic()
        if signature == self._last_signature and now - self._last_signature_time < 1.0:
            return
        ...
        for image in images:
            ...
            output_path = self.capture_directory / f"replacement_{self._capture_number:04d}.png"
        ...
        if added:
            ...
            self.state_changed.emit("Recording — copy more or finish")
    ...
    def _replace_inline_shape(old_image, replacement_file):
        ...
        new_image = document.InlineShapes.AddPicture(
            FileName=str(replacement_file), LinkToFile=False, SaveWithDocument=True, Range=old_range
        )
    ...
    def _replace_floating_shape(old_image, replacement_file):
        ...
        new_image = document.Shapes.AddPicture(
            FileName=str(replacement_file), LinkToFile=False, SaveWithDocument=True,
